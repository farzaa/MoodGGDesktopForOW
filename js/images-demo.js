...
var testImage = function(img) {
  ...
  for(var k=0;k<2;k++) {
    ...
    console.log("Choice...", classes_txt[preds[k].k]);
    console.log("Prob...", preds[k].p);
  }
...
var load_pretrained = function() {
    ...
    var directoryPath = location.substring(0, location.indexOf("index.html"));
    console.log(directoryPath + dataset_name + "_snapshot.json")
    $.getJSON(directoryPath + dataset_name + "_snapshot.json", function(json){
    net = new convnetjs.Net();
    net.fromJSON(json);
    console.log(trainer)
